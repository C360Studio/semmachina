...
- Add a minimal SvelteKit session surface over the existing authenticated `player/v1` WebSocket:
  submit one action, show typed progress/error/reconnect state, and receive or retrieve its result.
...
This is game-repository presentation and adapter work: a new SvelteKit surface, strict TypeScript
wire/view-model boundaries, server-only SemStreams GraphQL access, deterministic projection logic,
accessible components, and mock/real-stack acceptance coverage. It consumes the existing
authenticated player WebSocket for action submission, delivery, and retrieval, and the SemStreams
`v1.0.0-beta.159` GraphQL `entity`, `entitiesByPrefix`, `relationships`, and `spatialSearch`
operations for read projections. No engine payload, player protocol, graph predicate, rule pack,
component, or lifecycle contract changes. Any upstream gap discovered during implementation is
filed against SemStreams and remains visible as a blocker rather than being hand-rolled here.
