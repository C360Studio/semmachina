...
func (w *fakeDecisionWriter) Reconcile(
	_ context.Context, target projectioncontract.Target, _ string, triples []message.Triple,
) (*graph.EntityState, error) {
	if target != projectioncontract.TurnCaseDecision {
		return nil, errors.New("unexpected case decision projection target")
	}
